...
export default function CartItem({ item }) {
  ...
  const remove = () => removeFromCart(item.id);
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="flex justify-between">
            <div>
              <h3 className="text-sm sm:text-base font-semibold text-gray-800 dark:text-neutral-200">
                <Link href={`/product/${item.productId}`} className="hover:text-blue-600 dark:hover:text-blue-500">
                  {item.name}
                </Link>
              </h3>
          ...
          <div className="mt-auto pt-4 flex flex-wrap items-center justify-between gap-y-2">
            ...
            <button 
              ...
              onClick={remove}
              disabled={isRemoving}
              className="inline-flex items-center gap-x-1.5 text-sm font-medium text-gray-500 hover:text-red-500 focus:outline-none focus:text-red-500 disabled:opacity-50 disabled:pointer-events-none dark:text-neutral-500 dark:hover:text-red-500 dark:focus:text-red-500 transition-colors"
            >
